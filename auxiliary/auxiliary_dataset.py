...
class VideoDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if 'kinetics' in self.name:
            image_caption = torch.Tensor(random.choice(self.image_captions), dtype=torch.int64)
            ...
            if len(image_caption) < self.max_seq_len:
                image_caption = torch.cat((image_caption,
                                          ...
                                          dim=0)
            video_caption = torch.Tensor(random.choice(self.video_captions), dtype=torch.int64)
