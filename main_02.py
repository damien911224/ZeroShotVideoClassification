...
parser.add_argument('--bs',           default=4,   type=int,   help='Mini-Batchsize size per GPU.')
...
def train_one_epoch(train_dataloader, model, optimizer, embed_criterion, adversarial_criterion, opt, epoch):
    ...
    for i, (X, l, Z, _, image_captions) in enumerate(data_iterator):
        ...
        one_hot = F.one_hot(torch.maximum(image_captions, torch.Tensor(0)), 3000002).float()
        image_captions = torch.where(image_captions != -1, one_hot, torch.zeros_like(one_hot))
